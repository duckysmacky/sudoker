...
void SudokuSolver::solve()
{
    ...
    while (true)
    {
        if (!m_hide_grid && iteration % 1000 == 0) m_field.print_grid();

        // check if not out of bounds
        if (x < 0)
        {
            // move up the line if possible, else exit
            if (y > 0)
            {
                y--;
                x = 8;
            ...
            {
                std::cerr << "Unable to solve a Sudoku field: unable to find a valid solution" << std::endl;
                break;
            }
        }

        // move down the line when reached the end of the row
        if (x >= 9)
        {
            x = 0;
            y++;
        ...
        // skip pre-set numbers by moving in the current direction
        if (m_field.is_preset(y, x))
        {
            if (m_be_verbose) std::cout << "Skipping number at (" << y << "; " << x << "): number pre-set\n";
            if (can_place) x++; else x--;
            continue;
        }
        ...
        while (!(can_place = m_field.can_place(num, y, x)))
        {
            if (num >= 9) break;
            num++;
